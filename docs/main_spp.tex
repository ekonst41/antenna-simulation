...
\begin{abstract}
В данной работе численно исследуются моды поверхностных плазмон-поляритонов (SPP), локализованных на границе металл–диэлектрик, в многослойной структуре с различными типами диэлектриков, включая анизотропные материалы. Анализ проводится на основе решения уравнений Максвелла в частотной области, с расчётом комплексных волновых чисел, компонент электромагнитного поля и энергетического потока. Проект иллюстрирует ключевые принципы волновой оптики, включая поляризацию, интерференцию и локализацию энергии.
\end{abstract}
...
Интерес к SPP обусловлен их уникальными свойствами: сильной локализацией поля на границе, субволновым распространением и высокой чувствительностью к параметрам среды. Эти особенности делают поверхностные плазмон-поляритоны крайне перспективными для нанофотоники, сенсорных технологий, сверхразрешающей оптики и разработки метаматериалов.
...
Запишем это в виде матрицы, это так называемый  \href{https://en.wikipedia.org/wiki/Transfer-matrix_method_ (optics)}{Transfer Matrix Method}. Он используется для оптических систем с параллельными разделами слоев. В нем переход между слоями определяется матрицами:
...
Рассмотрим пример для 4 слоев, два из которых уходят на бесконечность. Для краткости, пусть $\delta_m = e^{i k_{zm} d_m}$. Тогда матрица системы:
...
Показатели диэлеткрической проницаемости для воздуха равны 1 по всем направлениям. Для золота диэлектричсекая проницаемость принята $\varepsilon_{Au}=-21.19+0.7361i$ по всем направляениям.
...
Характерным свойством такой структуры является ассиметрия до и после прохождения структуры
