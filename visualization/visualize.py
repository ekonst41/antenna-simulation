...
from physics.fields import calculate_Hy, calculate_Sx, calculate_Ez, calculate_Ex, calculate_Sz
...
def visualize(out, params, type):
    ...
    if 'S' in type:
        visualize_Sx(out, params)
    if 'Sz' in type:
        visualize_Sz(out, params)
...
def visualize_Sx(out, params, left_bound=-300, right_bound=500, num=400):
    plt.style.use('seaborn-v0_8-pastel')
...
def visualize_Sz(out, params, left_bound=-300, right_bound=500, num=400):
    plt.style.use('seaborn-v0_8-pastel')
    ...
    kx = params['kx']
    zs = np.linspace(left_bound * nu.nm, right_bound * nu.nm, num=num)
    Sx = np.array([calculate_Sz(z, out) for z in zs])


    Sx_real = Sx.real / max(abs(Sx.real))
    Sx_imag = Sx.imag / max(abs(Sx.imag))
    ...
    title = r"Распределение вектора Пойтинга $S_z$ для моды $k_x = {:.4f} + {:.4f}i$".format(
        kx.real / nu.um**-1,
        kx.imag / nu.um**-1
    )
    plt.title(title, pad=20, fontsize=14)
    ...
    plt.xlabel(r"$z$, нм", fontsize=12)
    plt.ylabel(r"Нормированная амплитуда $S_z$", fontsize=12)
